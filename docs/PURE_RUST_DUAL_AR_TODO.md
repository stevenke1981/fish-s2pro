...
- [x] **4.5** Golden test: single forward step hidden state L2 distance vs C++ dump (tight tolerance on CPU). Rust JSON dump generation, local s2.cpp JSON dump hook, and Rust/C++ comparator exist for the layer 0 single-token fixture plus a two-token, two-layer full-block prefill-style sequence fixture.
