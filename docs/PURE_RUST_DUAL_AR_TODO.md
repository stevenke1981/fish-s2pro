...
- [ ] `fish_s2_infer::pipeline::RustPipeline::synthesize(request) -> WAV`
  ...
  - [x] Full-path parity run: `.\scripts\dump_e2e_wav_parity.ps1 -MaxNewTokens 1` passes for greedy `hi` (`codes passed=true`, `compare-waveform passed=true`, WAV envelope `passed=true`).
...
- [x] **7.3** Reference conditioning: RustPure backend can encode `reference_wav` + `reference_text` into prompt codes at load time from `workdir/reference.*` and per request.
