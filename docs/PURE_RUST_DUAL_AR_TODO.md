...
- [ ] `fish_s2_infer::pipeline::RustPipeline::load(model_dir, config)`
  - [x] Pre-slice: `RustPipelineConfig` + `RustPipeline::load(transformer_gguf, codec_gguf, tokenizer)` loads tokenizer, `SlowArState`, Fast-AR weights, and codec decode weights.
  - [x] `fish_s2_e2e_wav_dump` CLI wraps `RustPipeline` for full-path smoke output (`generated_codes.json`, summary JSON, WAV).
  - Own tokenizer, transformer GGUF, codec GGUF, prompt/reference state.
  - Acceptance: can load `models/` without `s2.exe` or C++ lib. Next validation: run `fish_s2_e2e_wav_dump --max-new-tokens 1` against local GGUF and compare WAV envelope against s2.cpp full output.
- [ ] `fish_s2_infer::pipeline::RustPipeline::synthesize(request) -> WAV`
  - [x] Pre-slice: `RustSynthesisOptions` builds prompts, runs `generate_codes`, decodes codec waveform, and returns `RustSynthesisResult { codes, waveform, wav_bytes }`.
  - [x] Full-path parity harness: `scripts/dump_e2e_wav_parity.ps1` compares C++ generated codes, C++ waveform stats/WAV, and one-shot `RustPipeline` codes/stats/WAV outputs.
  - Calls tokenizer -> Slow-AR -> Fast-AR -> codec.
  - Acceptance: returns valid RIFF and passes Phase 0 envelope parity. Next validation: execute `.\scripts\dump_e2e_wav_parity.ps1 -MaxNewTokens 1` on the local GGUF pair.
