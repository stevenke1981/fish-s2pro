...
- [ ] **4.2** Implement prefill + decode step with **KV cache** (separate Slow-AR `gallocr` equivalent). **Prep slices complete:** `fish_s2_infer::tensor` now provides F16 typed tensor views plus scalar RMSNorm and `[in,out]` linear smoke tests; `fish_s2_infer::attention` now covers ggml normal RoPE, Slow-AR KV token writes, and single-step GQA decode attention smoke tests; `fish_s2_infer::slow_ar` now has a toy single-token layer skeleton that runs `rms_norm -> WQKV split -> QK norm -> RoPE -> KV write -> GQA attention -> output projection -> residual`, plus an ignored fixture that binds layer 0 to real local GGUF F16 tensors and checks finite single-token output. `fish_s2_slow_ar_dump` emits Rust JSON stats for the same fixture. `fish_s2_parity compare-slow-ar` now compares those JSON stats with strict per-tensor tolerances, so the remaining 4.5 work is adding the matching C++ dump source. Full prefill/decode parity is still pending.
...
- [ ] **4.5** Golden test: single forward step hidden state L2 distance vs C++ dump (tight tolerance on CPU). **Prep complete:** Rust JSON dump generation and Rust/C++ JSON comparator exist; matching s2.cpp JSON dump hook is still pending.
