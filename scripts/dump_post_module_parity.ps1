param(
    [string] $DumpDir = (Join-Path (Split-Path $PSScriptRoot -Parent) "output\s2cpp_post_module_dump"),
...
function Write-PostModuleDumpMain {
    param([string] $Path)
    $main = @'
#include "json.hpp"
...
// Keep the hook local to this generated helper. It reuses s2_codec.cpp static
// internals to stop exactly after build_transformer(quantizer.post_module).
...
void print_help() {
    std::cerr
        << "Usage: s2_post_module_dump --codec <codec.gguf> --codes <generated_codes.json> "
           "--output <post_module.json> [--threads 4]\n";
}
...
int main(int argc, char ** argv) {
    ...
    int32_t hidden_dim = 0;
    {
        ...
        try {
            hidden = s2::build_transformer(
                ctx,
                codec.impl_->ctx_w,
                codec.impl_->tprefix + "quantizer.post_module",
                stage_in,
                codec.impl_->rvq_transformer_block_size,
                codec.impl_->rvq_transformer_n_local_heads,
                codec.impl_->rvq_transformer_head_dim,
                codec.impl_->rvq_transformer_rope_base,
                codec.impl_->rvq_transformer_norm_eps,
                codec.impl_->rvq_transformer_window_size,
                inp);
            hidden = ggml_cpy(
                ctx,
                hidden,
            ...
            std::cerr << "post_module build failed: " << e.what() << "\n";
            ggml_free(ctx);
            return 1;
        }
        ...
        if (!allocr || !ggml_gallocr_alloc_graph(allocr, gf)) {
            std::cerr << "post_module graph allocation failed\n";
        ...
        if (ggml_backend_graph_compute(codec.impl_->backend, gf) != GGML_STATUS_SUCCESS) {
            std::cerr << "post_module compute failed\n";
...
function Write-PostModuleCMake {
    param(
        [string] $Path,
        [string] $SourceDir,
...
project(s2_post_module_dump LANGUAGES C CXX)
...
add_executable(s2_post_module_dump
    "$buildUnix/post_module_dump_main.cpp"
)

target_include_directories(s2_post_module_dump PRIVATE
    "`${S2_CPP_SRC}/include"
    "`${S2_CPP_SRC}/src"
...
target_link_libraries(s2_post_module_dump PRIVATE ggml)

if(MSVC)
    target_compile_options(s2_post_module_dump PRIVATE /EHsc /utf-8)
endif()
"@
    Write-Utf8NoBom $Path $cmake
}

$buildDir = Join-Path $DumpDir "build-cpu-post-module"

Write-PostModuleDumpMain (Join-Path $DumpDir "post_module_dump_main.cpp")
Write-PostModuleCMake `
...
cmake --build $buildDir --config $BuildType --target s2_post_module_dump --parallel
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$cppExe = Join-Path $buildDir "$BuildType\s2_post_module_dump.exe"
if (-not (Test-Path -LiteralPath $cppExe)) {
    $cppExe = Join-Path $buildDir "s2_post_module_dump.exe"
}
if (-not (Test-Path -LiteralPath $cppExe)) {
    throw "s2_post_module_dump not found under $buildDir"
}

$codeStem = [System.IO.Path]::GetFileNameWithoutExtension($Codes)
$tag = $codeStem -replace '^generated_codes_', ''
$cppJson = Join-Path $outDir "post_module_${tag}_cpp.json"
$rustJson = Join-Path $outDir "post_module_${tag}_rust.json"
...
cargo run -q -p fish_s2_parity -- compare-post-module $cppJson $rustJson
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host "post-module parity OK: $cppJson vs $rustJson"
