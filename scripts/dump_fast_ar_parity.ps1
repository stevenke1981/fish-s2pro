...
cmake --build $buildDir --config $BuildType --target s2_fast_ar_dump --parallel
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }

$cppExe = Join-Path $buildDir "$BuildType\s2_fast_ar_dump.exe"
if (-not (Test-Path -LiteralPath $cppExe)) {
    $cppExe = Join-Path $buildDir "s2_fast_ar_dump.exe"
}
if (-not (Test-Path -LiteralPath $cppExe)) {
    throw "s2_fast_ar_dump not found under $buildDir"
}

$tag = $Text -replace '[^a-zA-Z0-9]+', '_'
$cppJson = Join-Path $outDir "fast_ar_frame_${tag}_cpp.json"
$rustJson = Join-Path $outDir "fast_ar_frame_${tag}_rust.json"
...
cargo run --release -p fish_s2_infer --bin fish_s2_fast_ar_dump -- `
    --transformer $Transformer `
    --tokenizer $Tokenizer `
    --output $rustJson `
    --text $Text `
...
cargo run -p fish_s2_parity -- compare-fast-ar-frame $cppJson $rustJson
if ($LASTEXITCODE -ne 0) { exit $LASTEXITCODE }
Write-Host "Fast-AR frame parity OK: $cppJson vs $rustJson"