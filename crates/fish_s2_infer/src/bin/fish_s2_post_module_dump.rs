...
use fish_s2_infer::{
    forward_codec_post_module, models_dir, project_root, rvq_lookup_codes, CodecF16Weights,
    CodecPostModuleF16Weights, InferError,
};
...
fn main() -> fish_s2_infer::Result<()> {
    ...
    let dump = PostModuleDump {
        ...
        hidden_dim: post.hidden_dim,
        hidden_len: post.hidden.len(),
        hidden_l2: l2(&post.hidden),
        hidden_mean_abs: mean_abs(&post.hidden),
        hidden_max_abs: max_abs(&post.hidden),
        hidden_first8: post
            .hidden
            .iter()
            .take(8)
...
fn write_json(path: &Path, dump: &PostModuleDump) -> fish_s2_infer::Result<()> {
...
fn print_usage() {
    eprintln!(
        "Usage: fish_s2_post_module_dump [--codec codec.gguf] [--codes generated_codes.json] \
         [--output post_module.json]"
    );
}
